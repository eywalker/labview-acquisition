...
% available
function [xoffset effectivewidth yoffset effectiveheight zoffset aodframeislarger cancel] = FindMatch1(corr_data_aod, data_mirror, data_mirror_ch2,... 
                                                 pixelsize_aod, ...
...
% frames to use for rough alignment and intensity scaling
aodselframe = round(size(corr_data_aod,3)/2) ;    
mirrorselframe = round(size(scdata_mirror,3)/2) ;

% do the intensity scaling and rough alignment
[corr_data_aod scdata_mirror scdata_mirror_ch2 position cancel] = roughalignment(corr_data_aod, scdata_mirror, scdata_mirror_ch2, aodframeislarger) ;
